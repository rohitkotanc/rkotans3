...
int main(int argc, char* argv[])
{
    ...
    std::string outputFileName = "car-laptop-pole-circular-mobility.json";
    ...
    Ptr<WaypointMobilityModel> carMobility = nodes.Get(1)->GetObject<WaypointMobilityModel>();
    for (int i = 0; i < numWaypoints; ++i)
    {
        double angle = 2 * M_PI * i / numWaypoints;
        double x = radius * cos(angle);
        double y = radius * sin(angle);
        double time = i * (2 * M_PI * radius / speed) / numWaypoints;
    ...
    nodeHelper.Set("Model", netsimulyzer::models::LAPTOP_VALUE);
