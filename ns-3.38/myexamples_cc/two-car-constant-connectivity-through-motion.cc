...
int main(int argc, char* argv[])
{
    ...
    std::string outputFileName = "two-car-constant-connectivity-through-motion.json";
    ...
    positions->Add(Vector3D{25.0, 5.0, 0.0});
    positions->Add(Vector3D{5.0, 5.0, 0.0});
    positions->Add(Vector3D{15.0, -10.0, 0.0});
    ...
    mobility2->SetVelocity(Vector(0.0, 0.0, 0.0));
    PointToPointHelper pointToPoint;
    pointToPoint.SetDeviceAttribute("DataRate", StringValue("100Mbps"));
    pointToPoint.SetChannelAttribute("Delay", StringValue("0ms"));
    auto netDevices = pointToPoint.Install(nodes.Get(0), nodes.Get(1));
