...
void DropConnectivity(Ptr<NetDevice> clientDevice, Ptr<NetDevice> serverDevice)
{
    ...
    std::cout << "Connectivity drop (delay introduced) at: " << Simulator::Now().GetSeconds() << " seconds." << std::endl;
}

void RestoreConnectivity(Ptr<NetDevice> clientDevice, Ptr<NetDevice> serverDevice)
{
    Ptr<PointToPointNetDevice> clientP2PDevice = DynamicCast<PointToPointNetDevice>(clientDevice);
    Ptr<PointToPointNetDevice> serverP2PDevice = DynamicCast<PointToPointNetDevice>(serverDevice);
    clientP2PDevice->GetChannel()->SetAttribute("Delay", TimeValue(MilliSeconds(0)));
    serverP2PDevice->GetChannel()->SetAttribute("Delay", TimeValue(MilliSeconds(0)));

    std::cout << "Connectivity restored (delay removed) at: " << Simulator::Now().GetSeconds() << " seconds." << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    std::string outputFileName = "two-car-building-server-connection-dip.json"; 
    ...
    Simulator::Schedule(Seconds(6.0), &DropConnectivity, netDevices.Get(0), netDevices.Get(1));
    Simulator::Schedule(Seconds(7.0), &RestoreConnectivity, netDevices.Get(0), netDevices.Get(1));
    ...
    building->SetBoundaries(Box(15.0, 20.0, -10.0, -5.0, 0.0, 15.0));
